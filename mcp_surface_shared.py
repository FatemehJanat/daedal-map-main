...
def build_tool_definitions() -> list[dict]:
    return [
        ...
        },
        {
            ...
            "description": "Free geography utility. Converts a value from an external or adjacent geographic reference system into the DaedalMap loc_id universe. Examples: from_system='zip' value='00601'; from_system='nws_fire' value='AKZ317'; from_system='admin_boundary' value='Fairfax County'. Returns ranked loc_id matches with bridge vintage, overlap weights, and provenance where applicable. No payment required.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "loc_id_references",
            "title": "List loc_id References",
            "description": "Free geography utility. Returns known external or adjacent references for a DaedalMap loc_id, including local/geometry ids and catalog-backed side-chain overlaps such as ZCTAs, tribal areas, NWS public zones, and NWS fire weather zones. Use this when you already have a loc_id and need its equivalent or overlapping identifiers in other systems. No payment required.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "loc_id": {"type": "string", "description": "DaedalMap loc_id to inspect, such as USA-PR-001 or USA-AK-282."},
                    "systems": {"type": "array", "items": {"type": "string"}, "description": "Optional list of systems to include, such as zcta, nws_fire, overlay_tribal, or overlay_nws_public_zone."},
                    "iso3": {"type": "string", "description": "Optional country hint for bridge artifacts. Defaults to the loc_id country when possible."},
                    "target_admin_level": {"anyOf": [{"type": "string"}, {"type": "integer"}], "description": "Admin level of loc_id for bridge-backed reverse lookup. Inferred when omitted."},
                    "min_share": {"type": "number", "minimum": 0, "maximum": 1, "description": "Optional minimum target-area share for reverse overlap matches."},
        ...
        },
        {
            ...
            "description": "Free geography utility. Converts one geographic reference system into another by resolving through DaedalMap loc_id: X -> loc_id -> Y. Use this for workflows like ZIP/ZCTA to NWS fire zones, NWS zone to counties, county to overlapping ZCTAs, or any future catalog-backed reference bridge. No payment required.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        },
        {
            ...
            "description": "Free geography utility. Returns reusable geometry metadata for a DaedalMap loc_id: name, family, admin level, centroid, bounding box, and optional full GeoJSON polygon. Prefer bbox/centroid unless you need exact rendering or clipping. No payment required.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        },
        {
            ...
            "description": "Compatibility geography utility. Prefer resolve_reference for new workflows. Converts a side-chain geometry such as a Census ZCTA, tribal area, NWS public forecast zone, or NWS fire weather zone to a target administrative level using measured polygon overlap. No payment required.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "source_family": {"type": "string", "enum": ["overlay_zcta", "overlay_tribal", "overlay_nws_public_zone", "overlay_nws_fire_weather_zone"], "description": "Side-chain geometry family. Use overlay_zcta for Census ZCTAs, overlay_tribal for tribal areas, overlay_nws_public_zone for NWS public forecast zones, and overlay_nws_fire_weather_zone for NWS fire weather zones."},
                    "source_loc_id": {"type": "string", "description": "Side-chain loc_id. For ZCTAs, either canonical form such as 'USA-Z-10001' or a five-digit ZIP/ZCTA like '10001' is accepted. For NWS public forecast zones use canonical form such as 'USA-NWSZ-TXZ001'."},
