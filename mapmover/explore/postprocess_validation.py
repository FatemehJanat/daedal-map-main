"""Validation helpers extracted from the main postprocessor."""
...
def validate_item(
    ...
    *,
    ...
    if (
        ...
        and reroute_item_to_event_sibling_func(item, catalog)
    ):
    ...
    if source_requires_metric_func(item, catalog_source) and not metric:
        ...
        elif query_prefers_event_source_func(query) and reroute_item_to_event_sibling_func(item, catalog):
            return validate_item_func(item, catalog)
        else:
            item["_valid"] = False
