...
If an artifact exposes a `dimensions` block, treat those dimension columns and their declared values as first-class filter controls. Use them directly when the user asks for a subset like tract, block group, county, scenario, season, or category.
If an artifact exposes `geography_kind`, `admin_level_num`, or a dimension column such as `geo_level`, use those fields to respect requests like tract, block group, or block instead of inferring only from raw `loc_id` strings.
For mixed-geography artifacts, always filter on the artifact's explicit geography field before ranking or displaying results when the user asks for county, tract, block group, or block. Prefer a manifest-declared dimension column such as `geo_level` when present; otherwise use `geography_kind` or `admin_level_num`. Do not answer a tract request with block rows or a block request with tract rows.
When the manifest documents source-specific geography vocabularies, obey them exactly. Example: one artifact may use `geo_level=admin_3` while a sibling uses `geo_level=tract`; do not assume a shared vocabulary across the pack.
