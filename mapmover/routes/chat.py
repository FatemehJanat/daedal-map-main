...
from mapmover.logging_analytics import log_billing_fail_open_event
...
def _get_credit_balance(user_id: str) -> int:
    ...
    if not client:
        log_billing_fail_open_event(
            event_kind="credit_balance_fail_open",
            user_id=user_id,
...
def _deduct_credits(user_id: str, operation: str) -> dict:
    ...
    if not client:
        log_billing_fail_open_event(
            event_kind="credit_deduction_fail_open",
            user_id=user_id,
            operation=operation,
            reason="billing_not_configured",
        )
